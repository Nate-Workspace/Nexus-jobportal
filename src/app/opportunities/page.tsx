...
export default async function OpportunitiesPage({
  ...
  const currentType = getFilterValue(searchParams.type, "ALL") as
    | OpportunityType
    | undefined;
  const currentLocation = getFilterValue(searchParams.location, "ALL") as
    ...
    | "HYBRID"
    | undefined;
  const currentExperienceLevel = getFilterValue(
    searchParams.experienceLevel,
    "ALL"
  ) as ExperienceLevel | undefined; // New
  ...
  const initialOpportunitiesResponse = await graphqlClient<{
  ...
  const initialOpportunities =
    initialOpportunitiesResponse.data?.opportunities || [];

  const clientFetchMoreOpportunities = async (
    offset: number,
    limit: number
  ) => {
        ...
        <div className="grid gap-2">
          ...
          <Select
            onValueChange={(value) => {
              ...
              if (value && value !== "ALL")
                newSearchParams.set("experienceLevel", value);
